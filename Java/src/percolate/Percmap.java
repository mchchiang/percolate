...
class Percmap
{
	...
	int update()
	{
		...
		int numOfUpdates = 0;
		int i, j;
		for (i=1; i<newMap.length-1; i++)
		{
			for (j=1; j<newMap[i].length-1; j++)
			{
				if (map[i][j] != 0)
				{
					int max = map[i][j];
	...
	{
		/*
		 * Check if any number in the top level of the grid matches
		 * the ones at the bottom level 
		 */
		for (int i = 1; i < map.length-1; i++){
			if (map[1][i] != 0){
				int num = map[1][i];
				for (int j = 1; j < map.length-1; j++){
					if (num == map[map.length-2][j]){
						return true;
					}
				}
			}
